...
router.post("/api/play/games", requireLogin, requirePlayAccess, (req, res) => {
  ...
  const existing = db.prepare(`
    SELECT id, title FROM play_games WHERE bgg_game_id = ? OR bgg_url = ? OR normalized_title = ?
  `).get(bggGameId, bggUrl, normalizeName(title));

  if (existing) {
    return res.status(409).json({ error: "That game already exists", game: existing });
  }
...
router.get("/play/setup/:gameId", requireLogin, requirePlayAccess, (req, res) => {
  ...
  const defaultConfig = db.prepare(`
    SELECT * FROM play_game_configurations
...
router.post("/api/play/sessions", requireLogin, requirePlayAccess, (req, res) => {
  ...
  if (existing) {
    return res.status(409).json({ error: "You already have an active session", redirectUrl: `/play/session/${existing.session_key}` });
  }
  ...
  const finalMaxPlayers = Math.max(game.min_players || 1, Math.min(Number(maxPlayers) || game.max_players || 4, game.max_players || 4));
...
router.get("/play/session/:sessionKey", requireLogin, requirePlayAccess, (req, res) => {
  ...
  const mayView = session.visibility === "public" || session.host_user_id === user.id || !!hostIsFriend || !!member;
  ...
  if (!member) {
    ...
    const joinTx = db.transaction(() => {
      ...
      db.prepare(`
        UPDATE play_sessions SET current_players = current_players + 1 WHERE id = ?
      `).run(session.id);
    });
  ...
  const members = db.prepare(`
    ...
    WHERE m.session_id = ?
    ORDER BY CASE WHEN m.role = 'host' THEN 0 ELSE 1 END, COALESCE(u.display_name, u.username) COLLATE NOCASE
  `).all(session.id);
...
router.post("/api/play/sessions/:sessionId/state", requireLogin, requirePlayAccess, (req, res) => {
  ...
  db.prepare(`
    UPDATE play_session_state SET state_json = ?, updated_at = CURRENT_TIMESTAMP WHERE session_id = ?
...
router.post("/api/play/sessions/:sessionId/terminate", requireLogin, requirePlayAccess, (req, res) => {
  ...
  db.prepare(`
    UPDATE play_sessions SET status = ?, ended_at = CURRENT_TIMESTAMP WHERE id = ?
