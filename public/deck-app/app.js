...
function renderAbilityPool() {
  ...
  return `<div class="image-grid">${(state.abilityCategory.cards || [])
...
function renderZone(title, cards, pickType = '') {
  return `<section class="panel zone-panel"><h2>${title}</h2><div class="image-grid">${cards.length ? cards.map(card => `
...
function renderModifiers() {
  return `<section class="panel"><h2>Modifiers</h2>
    ...
    <div class="modifier-row">
      ...
      ${state.lastDrawnModifier ? `<img src="${cardImg(state.lastDrawnModifier)}" class="modifier-image" alt="${state.lastDrawnModifier.name}">` : ''}
    </div>
...
function renderGear() {
  ...
  const chosen = `<section class="panel"><h2>Chosen Gear</h2><div class="image-grid">${state.gearChosen.map(g => `
      <div class="gear-card">
  ...
  const available = `<section class="panel"><h2>Gear Library</h2>${state.allGear.map(cat => `
      <div><h3>${cat.name}</h3><div class="row-actions wrap">${cat.items.slice(0, 8).map(item => `<button class="btn small ${state.gearChosen.includes(item) ? 'active' : ''}" data-gear="${item.name}">${item.name}</button>`).join('')}</div></div>`).join('')}</section>`;
    ...
    <div class="deck-app shell">
      <div class="deck-toolbar">
        <div>
          ...
          <p class="muted">Assets served from /deck-assets → views/deck</p>
        </div>
        <div class="toolbar-actions wrap">
      ...
      <div class="two-col-grid">${renderModifiers()}${renderGear()}</div>
    </div>`;
  bindEvents();
}
